...
test("keeps the finished game free of starter preview code", async () => {
  ...
  assert.match(page, /COBRA_DURATION = 1\.18/);
  assert.match(page, /COBRA_DODGE_SPEED = 275/);
  assert.match(page, /type CobraVisual/);
  assert.match(page, /function getCobraVisual/);
  assert.match(page, /drawAircraftDepthOverlay/);
  assert.match(page, /drawCobraVapor/);
  ...
  assert.match(page, /return localToWorld\(aircraft, localX \+ cobra\.side \* cobra\.sideSlide \* 8, localY \* \(1 - cobra\.pitch \* 0\.18\)\)/);
  ...
  assert.match(page, /const flipScale = 1 - cobra\.flip \* 0\.46/);
  assert.match(page, /ctx\.scale\(\(1 - Math\.abs\(bank\) \* 0\.035\) \* flipScale/);
